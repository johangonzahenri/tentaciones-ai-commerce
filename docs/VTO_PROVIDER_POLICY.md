...
## 1. POLÃTICA DE SELECCIÃ“N DE PROVEEDORES

El motor de Virtual Try-On opera bajo una matriz determinista de selecciÃ³n de proveedores que impide el uso accidental o no autorizado de servicios de inferencia externos.
...
## 2. REGLA DE AISLAMIENTO PÃšBLICO

En el modo `PUBLIC_DEMO` (utilizado para el showcase de portafolio y despliegues estÃ¡ticos pÃºblicos):
- Toda peticiÃ³n se rutea de forma **inmutable** hacia el proveedor sintÃ©tico local `DemoVirtualTryOnProvider`.
- No se emite ninguna llamada HTTP hacia `api.fashn.ai`.
- Se rechaza la presencia de claves en el cliente para evitar exposiciÃ³n de credenciales comerciales.

---

## 3. ESPECIFICACIÃ“N DE MODELOS FASHN
...
1. **`tryon-max` (Recommended Preview Model):**
   ...
   - PropÃ³sito: MÃ¡xima resoluciÃ³n y fidelidad de texturas fotorrealistas en micro-estampados.
2. **`tryon-v1.6` (Production-Stable Model):**
   ...
   - PropÃ³sito: Alta disponibilidad, menor latencia y estabilidad para alto volumen transaccional.

---

## 4. GOBERNANZA DE CÃ“MPUTO Y CRÃ‰DITOS

Para todos los pilotos reales, el parÃ¡metro `num_images` se fija estrictamente en `1` para evitar sobreconsumo de cuotas de cÃ³mputo en la API de inferencia.
